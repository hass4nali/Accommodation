...
Class AdminEmail{
	public function AFEmail($email){
		$Db=new Database();
		$conn = $Db->Connect();
		$sql = "SELECT * FROM adminlogin WHERE Email = '".$email."'";  //selects email from database
		...
		if($numrows == 0) {
			echo ("Email does not exist! \n"); 
				...
				$sql = "UPDATE adminlogin SET ForgotPassword ='".$ResetCodeHash."', ForgotPasswordTime=DATE_ADD(NOW(),INTERVAL 5 MINUTE) WHERE Email = '".$email."'"; //updates logindetails with reset code and adds 5 mins on timer from the email given
						$result = $conn->query($sql);
				...
				header("Location: AdminResetPasswordForm.php"); //REDIRECTS USER TO RESET PASSWORD
		}
	}
	...
	public function ResetPass( $email, $ForgotPassword, $NewPassword ) {
			...
			$sql1 = "SELECT * FROM adminlogin WHERE Email = '" . $email . "'"; // selects email from database
			...
			if ( $numrows1 == 0 ) //checks if email exists
			{
				...
				if ( $hashcode == TRUE ) //checks if hashcode returns true
				{
					...
					$sql1 = "UPDATE adminlogin SET Password ='" . $hashpass . "' WHERE Email = '" . $email . "'"; //updates password in database with new password
					$result1 = $conn->query( $sql1 );
					echo( "Password Reset" );
				} else {
